...
// DEMO MODE - hardcode a user so login is bypassed
const DEMO_USER = { role: 'STUDENT', name: 'Demo User' }; // or 'INSTRUCTOR'

export const UserProvider = ({ children }) => {
    // DEMO MODE - backend bypassed for GitHub Pages
    const DEMO_USER = {
        user_id: 1,
    ...
    const courseSubmit = async () => ({ success: false, message: "Demo mode: backend unavailable." });
    const getClasses = async () => [];
    const getCard = async () => null;
    const getUserInfo = async () => "Demo User";

    return (
        <UserContext.Provider value={{ user, setUser, loading, error, courseSubmit, getClasses, getCard, getUserInfo }}>
            {children}
